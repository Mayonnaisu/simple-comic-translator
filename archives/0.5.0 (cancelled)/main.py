...
from app.core.image_utils import merge_images_vertically, slice_image_in_tiles, split_image_safely
from app.core.ocr import run_ocr_on_slices, deduplicate_results_2d, merge_nearby_boxes
...
if config:
    ...
    det_overlap = int(slice_height * config['DETECTION']['det_overlap'])
...
for dirpath, dirnames, filenames in natsorted(os.walk(args.input)):
    ...
    if merge_images:
        ...
        image_slices = slice_image_in_tiles(
            [merged_image, image_width, image_height], slice_height, slice_width, det_overlap, output_dir, log_level
        )
        detections = run_ocr_on_slices(image_slices, source_language, args.gpu, [use_slicer, h_stride, v_stride], "detection", log_level)

        higher_conf_detections = deduplicate_results_2d(detections, iou_threshold=0.2, stage=1)

        unique_detections = deduplicate_results_2d(higher_conf_detections, iou_threshold=0.2, stage=2)

        merged_detections = merge_nearby_boxes(unique_detections, det_y_threshold, det_x_threshold)
    ...
    overlay_translated_texts(image_chunks, merged_detections, output_dir)
